...
def _merge_provider(grouped: dict[str, dict[str, Any]],
    ...
    """把单个上游的 {小写名: 原始 id} 合并进 grouped / aliases。"""
    provider_aliases = aliases.setdefault(provider_id, {})
    for lower, original in name_map.items():
        provider_aliases[lower] = original
        entry = grouped.setdefault(lower, {"canonical": original, "providers": set()})
        # canonical 偏向全小写形式（与 OpenAI 惯例一致）
        if original == original.lower():
            entry["canonical"] = original
