...
- **部署契约**：前端产物改动后刷新即生效；**后端 `src/` 改动必须重启进程**——`launchd`/systemd/docker 都不监听源码，`KeepAlive` 不是热重载。两者独立更新会产生「新前端 + 旧后端」错配（新端点 `404` → 前端报无关的兜底文案），故升级后必须重启（详见 [TECHNICAL.md §6.4](TECHNICAL.md) 与 [README.md「部署注意」](README.md)）
