...
| 成长中心 `/v2/activity/growth/streak` | `streak.days` | 连登天数：含 1 天容忍窗口（`days = 客户端使用天数 + 1`），每月清零 |
...
**什么算「使用」——这里有实测数据，别再凭单次实验下结论**：

| 日期 | 代理聊天转发 | 成长中心领取 | `score` |
|---|---|---|---|
| 09-18 | 360 + 253 次 | 未部署（无领取） | **0** |
| 09-19 | 391 + 202 次 | 有（21:32 起领旅行礼物） | **5** |
| 09-20 | 110 + 227 次 | 有 | 0 / 2（另有一次桌面端对话） |

两天都没有桌面端使用，唯一的差别就是「有没有成长中心的领取动作」——**活动类接口
操作计入活跃**。而单纯走 `/v2/chat/completions` 发对话不计入（实测 3 次完整对话
跑到 `[DONE]`，`today.score` 与 `updated_at` 都不动）。

本项目代码注释与文档里一度写死「只计官方客户端、代理一律不计」，那是**依据单次
实验得出的错误结论**（拿"聊天不计入"推广成了"代理全部不计入"），已更正。

**由此得到的边界**：活动类操作（领取奖励、兑换、开盲盒）可以由本项目管理；
而依赖**产品内真实功能使用**的任务（如 `first_buddy` = 「在客户端新建任务并发起
对话」）必须在官方客户端完成——这不是接口限制，是任务定义本身要求的行为。
